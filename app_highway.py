...
try:
    print(f"HIGHWAY_APP: Loading YOLO model from: {YOLO_MODEL_PATH}")
    model = YOLO(YOLO_MODEL_PATH)
    print("HIGHWAY_APP: YOLO model loaded successfully.")
except Exception as e:
    print(f"HIGHWAY_APP: Error loading YOLO model: {e}")
    model = None
...
def generate_frames(video_path, params_dict):
    ...
    middle_line_y = int(processing_height * 4/5)
            ...
            for box_data, tid, cls, conf in zip(boxes_data, ids_data, clss_data, confs_data): # Mengganti box menjadi box_data
                ...
                if abnormal_count[tid] >= params_dict['ACCIDENT_THRESHOLD']:
                    if tid not in counted_accidents:
                        ...
                        lokasi_kejadian = "Monitoring Jalan Raya (highway.mp4)"
                        pesan_notifikasi = (
                        ...
                        print(f"HIGHWAY_APP: Notifikasi insiden untuk ID {tid} dari '{lokasi_kejadian}' coba dikirim ke Telegram.")
...
def video_feed_highway():
    # Parameter HIGHWAY_PARAMS sekarang menggunakan nilai yang lebih masuk akal
    # yang telah disesuaikan dari log user sebelumnya
    print("HIGHWAY_APP: Memulai stream untuk /video_feed_highway dengan parameter:", HIGHWAY_PARAMS)
    return Response(generate_frames(video_path='highway.mp4', params_dict=HIGHWAY_PARAMS),
                    mimetype='multipart/x-mixed-replace; boundary=frame')

if __name__ == '__main__':
    PORT_HIGHWAY = 5001
    print(f"HIGHWAY_APP: Memulai Flask server dengan model: {YOLO_MODEL_PATH}, lebar proses: {PROCESSING_WIDTH}px, kualitas JPEG: {JPEG_QUALITY}")
    print(f"HIGHWAY_APP: Parameter untuk highway video: {HIGHWAY_PARAMS}") # HIGHWAY_PARAMS digunakan di sini
    print(f"HIGHWAY_APP: Akses stream di http://localhost:{PORT_HIGHWAY}/video_feed_highway atau http://<IP_LOKAL_ANDA>:{PORT_HIGHWAY}/video_feed_highway")
    app.run(host='0.0.0.0', port=PORT_HIGHWAY, debug=False, threaded=True, use_reloader=False)